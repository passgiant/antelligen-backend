...
class IncrementalCollectUseCase:
    def __init__(
        self,
    ...
    async def execute(self) -> DisclosureCollectionResponse:
        job = await self._job_repo.save_job(
            CollectionJob(
                job_name="incremental_collect",
                job_type="hourly",
        ...
        try:
            # 1. 마지막 수집 날짜 확인
            latest_dt = await self._disclosure_repo.find_latest_rcept_dt()

            if latest_dt is None:
                bgn_date = (datetime.now() - timedelta(days=7)).strftime("%Y%m%d")
            else:
                bgn_date = latest_dt.strftime("%Y%m%d")
